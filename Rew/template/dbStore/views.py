...
@csrf_exempt
def apertureApi(request):
    liststore = list()
    data = json.loads(request.body)
    if(data['search'].isnumeric()):
        store_all = Aperture.objects.filter(aper_price__contains=data['search']).values()
    else:
        store_all = Aperture.objects.filter(aper_loc__contains=data['search']).values()
        
    for i in store_all:
        if(i['aper_status'] == False):
            liststore.append(i)
    print(liststore)
